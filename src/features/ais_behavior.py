...
def compute_position_jumps(df: pl.DataFrame) -> pl.DataFrame:
    ...
    sorted_df = df.sort(["mmsi", "timestamp"]).with_columns([
...
def compute_sts_candidates(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if stopped.is_empty():
        return pl.DataFrame({"mmsi": [], "sts_candidate_count": []},
...
def compute_loitering(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        df.lazy()
        .filter(
            (pl.col("sog") < LOITER_SPEED_KNOTS) &
            (pl.col("nav_status") != PORT_MOORED_STATUS)
        )
        .sort(["mmsi", "timestamp"])
        .with_columns(
            pl.col("timestamp").diff().over("mmsi")
            .dt.total_minutes().alias("delta_min")
        )
...
def compute_port_call_ratio(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .group_by("mmsi")
        .agg([
            (pl.col("nav_status") == PORT_MOORED_STATUS).sum().cast(pl.Float32)
            .alias("moored"),
            pl.len().cast(pl.Float32).alias("total_stopped"),
        ])
        .with_columns(
            (pl.col("moored") / pl.col("total_stopped")).alias("port_call_ratio")
        )
